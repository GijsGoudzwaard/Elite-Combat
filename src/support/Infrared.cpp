...
// volatile uint8_t dataTBS = 181; // dataToBeSend is stored in data, during start bit; This way the data isnt corrupted during a transmission.
...
//     TCCR2A = (1 << WGM20) | (1 << COM2B1);// Set Phase Correct PWM Mode, Clear OC2B on Compare Match when up-counting. Set OC2B on Compare Match when down-counting.
//     TCCR2B = (1 << CS20) | (1 << WGM22);   // No prescaler, Set Phase Correct PWM Mode. 
...
//     if (kHz == 38)
//     {
//         OCR2A = 210; // Setting PWM on 38kHz
//     } else 
//     {
//         OCR2A = 138; // Setting PWM on 58kHz
//     }
...
//         if (!bit)
//         {
//             TCCR2A &= ~(1 << COM2B1); // Send a signal when there is a 1 bit, so the receiver reads a 1
//         }
//         else 
//         {
//             TCCR2A |= (1 << COM2B1); // Send no signal when there is a 0 bit, so the receiver reads a 0
//         }
//     }
//     if (!startBit)
//     {
...
// {
//     time2Counter++;
//     if (time2Counter == 20*kHz) // The 2 arduino's are operating at different speeds. (58kHz and 38kHz) Parameter 20 can be changed accordingly
//     {               
//         timerDataReceive(); // Calling the function to check for incoming data
//         timerDataSend(); // Calling the function to send data
//         time2Counter = 0; // Reset the time2Counter for a continious meassuring of time
//     }
// }